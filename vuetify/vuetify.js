...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    computed: {
        pageSize: function pageSize() {
            var len = this.computedItems.length;
        ...
        virtualizedItems: function virtualizedItems() {
            var firstItem = this.firstItemIndex || 0;
            var lastItem = this.computedItems.length;
            if (firstItem + this.pageSize < lastItem) {
                lastItem = firstItem + this.pageSize;
            }
            // this only for test passing
            return this.$_menuProps.auto ? this.computedItems : this.computedItems.slice(firstItem, lastItem);
            // need use this
    ...
    watch: {
        ...
        isMenuActive: function isMenuActive(val) {
            ...
            this.$nextTick(function () {
                if (_this.content && _this.content.addEventListener) {
                    setTimeout(function () {
                        var offsetY = (_this.firstItemIndex + 4) * _this.virtualItemHeight;
                        _this.content.scrollTop = offsetY > _this.computedItems.length * _this.virtualItemHeight ? _this.computedItems.length * _this.virtualItemHeight : offsetY < 0 ? 0 : offsetY;
        ...
        scrollSelectionTop: function scrollSelectionTop(val, oldVal) {
            if (val !== oldVal) {
                var scrollDirection = val > oldVal;
                var firstOffset = this.virtualItemHeight * this.firstItemIndex;
                var needChange = scrollDirection ? val - firstOffset > this.virtualItemHeight * this.pageSize * 0.33 : val - firstOffset < this.virtualItemHeight * this.pageSize * 0.33;
                if (needChange) {
                    // (scrollDirection ? 1 : -1) *
                    var firstItemIndexValue = val / this.virtualItemHeight - (scrollDirection ? this.pageSize * 0.15 : this.pageSize * 0.66) >> 0;
    ...
    methods: {
        ...
        setSelectedItems: function setSelectedItems() {
            ...
            if (firstIdx > -1) {
                this.$nextTick(function () {
                    firstIdx = firstIdx - 6;
                    _this.firstItemIndex = firstIdx > 0 ? firstIdx : 0;
                });
            }
...
/* harmony default export */ __webpack_exports__["default"] = ({
    ...
    methods: {
        ...
        genTile: function genTile(itemIndex, item, disabled, avatar, value) {
            var _this = this;
            if (disabled === void 0) {
                disabled = null;
            }
            ...
            var translateYValue = this.firstItemIndex * this.itemHeight + itemIndex * this.itemHeight;
            var tile = {
                on: {
                    mousedown: function mousedown(e) {
                        // Prevent onBlur from being called
    ...
    render: function render() {
        ...
        for (var itemIndex in this.items) {
            ...
            if (item == null) children.push(this.genTile(itemIndex, item));else if (item.header) children.push(this.genHeader(item));else if (item.divider) children.push(this.genDivider(item));else children.push(this.genTile(itemIndex, item));
        }
